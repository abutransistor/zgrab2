...
var progress_success_counter = 0
var progress_counter = 0
// grabTarget calls handler for each action
func grabTarget(input ScanTarget, m *Monitor) []byte {
	moduleResult := make(map[string]ScanResponse)
	var isIncrementedProgress = false
	var isIncrementedSuccess = false
	...
	for _, scannerName := range orderedScanners {
		...
		if !isIncrementedProgress {
			isIncrementedProgress = true
			progress_counter++
			progress_counter_file, err := os.OpenFile("/var/log/zgrab2.progress", os.O_WRONLY|os.O_CREATE, 0666)
			if err != nil {
				log.Errorf("Error opening progress file: %v", err)
			} else {
				defer progress_counter_file.Close()
				progress_counter_file.WriteString(strconv.Itoa(progress_counter))
			}
		}
		
		if res.Status == SCAN_SUCCESS && !isIncrementedSuccess {
			isIncrementedSuccess = true
			//increment progress counter and write to file
			progress_success_counter++
			progress_success_file, err := os.OpenFile("/var/log/zgrab2.success.progress", os.O_WRONLY|os.O_CREATE, 0666)
			if err != nil {
				log.Errorf("Error opening progress file: %v", err)
			} else {
				defer progress_success_file.Close()
				progress_success_file.WriteString(strconv.Itoa(progress_success_counter))
			}
		}
	}
